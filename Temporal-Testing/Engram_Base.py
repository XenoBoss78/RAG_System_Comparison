...
class OllamaEmbedder(Embedder):
    ...
    def ensure_dimension(self) -> int:
        """Initialize ``dim`` before reopening a persisted embedding store."""
        if self.dim <= 0:
...
def load_document_ids_file(path: Path) -> set[str]:
    """Load a JSON ID list/object or a one-document-ID-per-line text file."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Build from only the document IDs in this JSON list (or a JSON object with "
