...
DEFAULT_DB_DIR = FIN_RATE_DIR / "vector_db"
...
def retrieval_pipeline(query: str, *, seed: int = DEFAULT_SEED) -> list[str]:
    ...
    ranked = np.lexsort((positive, -scores_array[positive]))
    top_positions = positive[ranked[:top_k]]
...
def retrieve_relevant_chunks(query: str, *, seed: int = DEFAULT_SEED) -> list[str]:
    return retrieval_pipeline(query, seed=seed)
...
def _main() -> None:
    ...
    if args.query:
        ...
        for chunk in retrieval_pipeline(args.query, seed=args.seed):
            print(chunk)
            print("\n---\n")
