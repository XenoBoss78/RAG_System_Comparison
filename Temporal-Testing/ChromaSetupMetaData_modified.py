...
# These are soft ranking hints, not database filters.  They let a temporal
# retrieval keep potentially useful filing types while preferring the section
# most likely to contain the requested evidence.
SECTION_SIGNAL_RULES: tuple[tuple[tuple[str, ...], tuple[str, ...]], ...] = (
    (("audit fee", "audit fees", "auditor", "principal accountant"), ("audit fee", "audit-related fee", "principal accountant", "fees paid")),
    (("employee", "employees", "headcount", "workforce", "collective bargaining", "human capital"), ("employees", "human capital", "collective bargaining", "workforce", "headcount")),
    (("compensation", "salary", "bonus", "stock award", "executive pay"), ("executive compensation", "salary", "bonus", "stock award", "summary compensation")),
    (("board", "director", "chairperson", "chairman", "governance"), ("board of directors", "director", "chairperson", "governance")),
    (("vesting", "equity award", "incentive plan", "restricted stock"), ("vesting", "equity award", "incentive plan", "restricted stock")),
    (("cyber", "cybersecurity", "data protection", "privacy", "private key", "wallet"), ("cybersecurity", "data security", "privacy", "private key", "wallet", "risk factor")),
    (("risk factor", "international expansion", "shipping"), ("risk factor", "international expansion", "shipping")),
    (("revenue", "operating loss", "net loss", "profitability"), ("revenue", "operating loss", "net loss", "results of operations")),
    (("market risk", "foreign exchange", "commodity price", "hedge"), ("market risk", "foreign exchange", "commodity price", "hedge")),
)

FORM_PREFERENCE_RULES: tuple[tuple[tuple[str, ...], tuple[str, ...]], ...] = (
    (("10-q", "quarter", "quarterly"), ("10-Q",)),
    (("proxy", "def 14a", "executive compensation", "director compensation"), ("DEF 14A", "DEFM14A", "PRE 14A")),
    (("10-k", "annual report", "risk factor", "market risk", "human capital"), ("10-K", "20-F")),
)
...
def _metadata_filter_from_hints(hints: dict[str, Any]) -> dict[str, Any] | None:
    """Convert already-extracted query hints into the legacy combined where filter."""
    conditions: list[dict[str, Any]] = []
    company_condition = _company_where_condition(
        [str(ticker) for ticker in hints.get("tickers", []) if str(ticker)]
    )
    if company_condition is not None:
        conditions.append(company_condition)
    year_condition = _year_where_condition(
        sorted({int(year) for year in hints.get("years", [])})
    )
...
def _query_metadata_hints(
    ...
    *,
    ...
    """Return canonical company tickers and requested years for staged retrieval."""
    ...
    if normalized_mode == "ollama":
        ...
        return {
            ...
            "years": sorted({int(year) for year in breakdown.get("years", [])}),
        }
...
def _section_terms_for_query(query: str) -> set[str]:
    """Infer small, high-precision section terms for a post-retrieval boost."""
    lowered_query = query.lower()
    terms: set[str] = set()
    for triggers, section_terms in SECTION_SIGNAL_RULES:
        if any(trigger in lowered_query for trigger in triggers):
            terms.update(section_terms)
    return terms


def _preferred_forms_for_query(query: str) -> set[str]:
    """Return filing forms to prefer softly when the question is explicit."""
    lowered_query = query.lower()
    forms: set[str] = set()
    for triggers, preferred_forms in FORM_PREFERENCE_RULES:
        if any(trigger in lowered_query for trigger in triggers):
            forms.update(preferred_forms)
    return forms


def _section_and_form_boost(
    chunk: dict[str, Any],
    *,
    section_terms: set[str],
    preferred_forms: set[str],
    section_boost: float,
) -> float:
    """Score an already-retrieved chunk using title/section and filing-form cues."""
    if section_boost <= 0:
        return 0.0
    metadata = chunk.get("metadata") or {}
    title = str(metadata.get("title") or "").replace("_", " ").lower()
    text_prefix = str(chunk.get("text") or "")[:2500].lower()
    searchable = f"{title}\n{text_prefix}"
    matched_terms = sum(1 for term in section_terms if term in searchable)
    # Cap the boost so vector similarity remains the primary ordering signal.
    boost = min(section_boost, 0.06 * matched_terms)
    filing_form = str(metadata.get("filing_form") or "").upper()
    if preferred_forms and filing_form in preferred_forms:
        boost += min(0.04, section_boost / 2)
    return boost


def _chunks_from_chroma_results(results: dict[str, Any]) -> list[dict[str, Any]]:
    """Normalize a Chroma query response to the public chunk result shape."""
...
def _multilayer_temporal_retrieve(
    collection: Any,
    query: str,
    *,
    n_results: int,
    ...
    """
    Retrieve each requested year independently, then merge a balanced evidence set.

    Returning None tells the caller to use the legacy one-shot metadata search:
    staged retrieval is only safe when a canonical company and at least two years
    are available.
    """
    tickers = [str(ticker) for ticker in hints.get("tickers", []) if str(ticker)]
    years = sorted({int(year) for year in hints.get("years", [])})
    if len(tickers) != 1 or len(years) < 2:
        return None

    company_where = _company_where_condition(tickers)
    base_where = _merge_where_filters(where, company_where)
    section_terms = _section_terms_for_query(query)
    preferred_forms = _preferred_forms_for_query(query)
    candidate_count = min(
        collection_count,
        max(n_results, n_results * max(1, candidate_multiplier)),
    )
    per_year_quota = max(1, n_results // len(years))
    candidates_by_year: dict[int, list[dict[str, Any]]] = {}

    def query_chroma(active_where: dict[str, Any], target_year: int) -> list[dict[str, Any]]:
        query_kwargs: dict[str, Any] = {
            "query_texts": [f"{query.strip()} Reporting period {target_year}."],
        ...
        chunks = _chunks_from_chroma_results(collection.query(**query_kwargs))
        for chunk in chunks:
            raw_score = float(chunk.get("score") or 0.0)
            boost = _section_and_form_boost(
                chunk,
            ...
            chunk["section_form_boost"] = boost
            chunk["score"] = raw_score + boost
            chunk["target_period_year"] = target_year
        return sorted(chunks, key=lambda chunk: float(chunk.get("score") or 0.0), reverse=True)

    for year in years:
        year_where = _merge_where_filters(base_where, _year_where_condition([year]))
        year_chunks = query_chroma(year_where, year)
        # Preserve the company constraint when a filing/chunk has incomplete year metadata.
        if not year_chunks and not strict_metadata_filter and base_where is not None:
            year_chunks = query_chroma(base_where, year)
            for chunk in year_chunks:
                chunk["temporal_filter_fallback"] = "company_only"
        candidates_by_year[year] = year_chunks

    if not any(candidates_by_year.values()):
        return None

    selected: list[dict[str, Any]] = []
    selected_ids: set[str] = set()
    year_offsets = {year: 0 for year in years}
    # Interleave periods so an answer prompt reaches both endpoints early rather
    # than consuming its context window on the first year alone.
    for _ in range(per_year_quota):
        for year in years:
            candidates = candidates_by_year[year]
            while year_offsets[year] < len(candidates):
                chunk = candidates[year_offsets[year]]
                year_offsets[year] += 1
    ...
    remaining = sorted(
        (chunk for chunks in candidates_by_year.values() for chunk in chunks),
    ...
    for chunk in selected:
        ...
        metadata["multilayer_temporal_filter"] = True
        metadata["retrieval_target_year"] = int(chunk.get("target_period_year"))
...
def retrieve_relevant_chunks(
    ...
    *,
    ...
    multilayer_temporal_filter: bool = True,
    multilayer_section_boost: float = 0.12,
    multilayer_candidate_multiplier: int = 3,
    corpus_path: str | Path = DEFAULT_CORPUS_PATH,
    seed: int = DEFAULT_SEED,
) -> list[dict[str, Any]]:
    """
    ...
    if multilayer_section_boost < 0:
        raise ValueError("multilayer_section_boost must be non-negative")
    if multilayer_candidate_multiplier <= 0:
        raise ValueError("multilayer_candidate_multiplier must be positive")
    ...
    auto_where = _metadata_filter_from_hints(hints) if auto_metadata_filter else None
    effective_where = _merge_where_filters(where, auto_where)

    if auto_metadata_filter and multilayer_temporal_filter:
        staged_chunks = _multilayer_temporal_retrieve(
            collection,
            query,
            n_results=n_results,
            ...
            section_boost=multilayer_section_boost,
            candidate_multiplier=multilayer_candidate_multiplier,
        )
    ...
    return _chunks_from_chroma_results(results)
...
def answer_question(
    ...
    *,
    ...
    chunks = retrieve_relevant_chunks(
        ...
        ollama_url=ollama_url,
        strict_metadata_filter=strict_metadata_filter,
        multilayer_temporal_filter=multilayer_temporal_filter,
        multilayer_section_boost=multilayer_section_boost,
        multilayer_candidate_multiplier=multilayer_candidate_multiplier,
        corpus_path=corpus_path,
        seed=seed,
    )
...
def _main() -> None:
    ...
    parser.add_argument(
        "--no-multilayer-temporal-filter",
        action="store_true",
        help=(
            "Use the legacy one-shot metadata query instead of independent per-year "
            "retrieval, section-aware reranking, and balanced merging."
        ),
    )
    parser.add_argument(
        "--multilayer-section-boost",
        type=float,
        default=0.12,
        help="Maximum soft reranking boost for likely filing-section terms (default: 0.12).",
    )
    parser.add_argument(
        "--multilayer-candidate-multiplier",
        type=int,
        default=3,
        help="Per-year candidate pool multiplier before balanced merging (default: 3).",
    )
    ...
    if args.query:
        chunks = retrieve_relevant_chunks(
            ...
            multilayer_temporal_filter=not args.no_multilayer_temporal_filter,
            multilayer_section_boost=args.multilayer_section_boost,
            multilayer_candidate_multiplier=args.multilayer_candidate_multiplier,
            corpus_path=args.corpus,
            seed=args.seed,
        )
        for chunk in chunks:
